      ...
      <div className="w-full max-w-sm animate-rise">
        ...
        <div className="rounded-2xl border border-line bg-panel/60 backdrop-blur p-6">
          ...
          <p className="text-fog text-sm mt-1 mb-6">
            {mode === "in" ? COPY.login.subIn : COPY.login.subUp}
          </p>

          <label className="block text-xs text-fog mb-1.5">{COPY.login.email}</label>
          ...
          <label className="block text-xs text-fog mb-1.5">{COPY.login.password}</label>
        ...
        <p className="text-center text-[11px] text-fog/60 mt-6 tracking-wide">{COPY.login.footer}</p>
      </div>
    </div>
  );
}

    ...
    <div className="min-h-screen pb-24 md:pb-0 md:pl-56">
      <aside className="hidden md:flex fixed left-0 top-0 h-screen w-56 flex-col border-r border-line bg-panel/40 backdrop-blur px-4 py-6">
        ...
        <button onClick={() => supabase.auth.signOut()}
          className="mt-auto text-left text-xs text-fog hover:text-ember px-3 py-2">{COPY.nav.logout}</button>
      </aside>
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <div className="leading-none">
        <div className="font-display font-extrabold tracking-tight text-bone">{COPY.brandName}</div>
        <div className="text-[10px] text-fog tracking-[0.2em] uppercase">{COPY.brandTag}</div>
      </div>
    </div>
  );
}
